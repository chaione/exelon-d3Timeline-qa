...
var _LOCATIONS = []
var _HAS_SUBSTEP_LOCATIONS = ['Sierra 1', 'Sally Port']
...
var _DS = {
  ...
  routes: [
    ...
    {
      name: 'warehouse_and_pa',
      order: ['S1', 'SG', 'WH', 'SP', 'PA', 'SP', 'SG', 'EX']
    }
  ],
  ...
  EVENTS_META: [
    ...
    {
      name: 'sp_vvro_release_vehicle_exiting',
      to: ['vvro'],
      responsible: true
    },
...
var yesterdayMidDay = moment()
  .subtract(1, 'days')
  .startOf('day')
  .set('hour', 12)
var tomorrowMidDay = moment()
  .add(1, 'days')
  .startOf('day')
  .set('hour', 12)
...
var xScale = d3.time.scale
  .utc()
  .domain([yesterdayMidDay.unix() * 1000, tomorrowMidDay.unix() * 1000])
  .range([0, _X_AXIS_WIDTH])

var yDeliveryScale = d3.scale
  .linear()
  .domain([1, 7])
  .range([1 + rowHeight, 7 * rowHeight])

var viewportScale = d3.time.scale
  .utc()
  .domain([
...
startingX = viewportScale(
  new Date(nowYear, nowMonth, nowDay, nowHours, nowMinutes)
)
...
function setupSvgStructure() {
  ...
  var maskingGradientHorizontal = svgDefs
    .append('linearGradient')
  ...
  aheadGradient
    ...
    .attr('offset', '1')
  var lateGradient = svgDefs.append('linearGradient').attr('id', 'lateGradient')
  lateGradient
    .append('stop')
  ...
  onTimeGradient
    ...
    .attr('offset', '1')
  var deniedEntryGradient = svgDefs
    .append('linearGradient')
  ...
  yAxisGroup = deliveriesGroup.append('g').attr('class', 'y axis')
  ...
  xAxisMask = g
    ...
    .attr(
      'transform',
      'translate(' + 0 + ',' + (outerHeight - _X_AXIS_HEIGHT * 2 + 1) + ')'
    )

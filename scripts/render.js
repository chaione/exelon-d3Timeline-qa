...
function moveXAxis(a, b) {
  ...
  deliveriesGroup.attr(
    'transform',
    'translate(' + [eventxTranslation, eventyTranslation] + ')scale(1)'
  )

  stationsGroup.attr(
    'transform',
    'translate(' + [0, eventyTranslation] + ')scale(1)'
  )
  delieveryStaticGroup.attr(
    'transform',
    'translate(' + [0, eventyTranslation] + ')scale(1)'
  )
  ...
  xAxisGroup.attr(
    'transform',
    'translate(' + [eventxTranslation, innerHeight] + ')scale(1)'
  )
  if (_DS.isDetailDisplayed) {
    detailDeliveryDataGroup.attr(
      'transform',
      'translate(' + [eventxTranslation, detailDeliveryRectY] + ')scale(1)'
    )
  }
}
...
function render(data) {
  ...
  var stationsLabelSelectAll = stationsGroup
    .selectAll('.station')
    .data(_DS.locations)
  var stationsLabelSelectAllG = stationsLabelSelectAll
    .enter()
    .append('g')
    .attr('class', 'station') // stations should never exit

  stationsLabelSelectAllG.append('rect').attr('class', 'stationRect')
  var stationsLabelStationRectSelectAll = stationsLabelSelectAll.selectAll(
    '.stationRect'
  )
  ...
  stationsLabelStationRectSelectAll
    ...
    .attr('y', function(d, i) {
      return d.y0 * rowHeight + rowHeight / 2
    })
    .attr('width', innerWidth)
    .attr('height', function(d, i) {
      return d.deliveryCount * rowHeight
    })

  stationsLabelSelectAllG
    .append('text')
    .attr('x', function(d, i) {
  ...
  stationsLabelSelectAll
    .selectAll('.deliveryBGStatuss') // denying entry bg
    ...
    .attr('transform', function(d) {
      return (
        ...
        0 +
        ',' +
        (yDeliveryScale(d.yIndex + 1) - rowHeight / 2) +
        ')'
      )
    })

  var delieveryStaticGroupSelectAll = delieveryStaticGroup
    .selectAll('.deliveryStatuss') // denying entry text
    ...
    .attr('y', function(d, i) {
      return yDeliveryScale(d.yIndex + 1) + 6
    }) // why is the 6 needed???
    .text(function(d, i) {
      if (d.status === 'denied') {
  ...
  // Setup Stations
  var stationsSelectAll = deliveriesGroup
    .selectAll('.station')
  ...
  // Setup Deliveries
  var deliveriesSelectAll = stationsSelectAll
    .selectAll('.delivery')
    .data(function(d) {
      return d.values
    })
  ...
  // The delivery group
  var deliveriesSelectAllG = deliveriesSelectAll
    .enter()
    .append('g')
    .attr('class', function(d) {
  ...
  deliveriesSelectAllG.attr('transform', function(d) {
    return 'translate(' + 0 + ',' + yDeliveryScale(d.yIndex + 1) + ')'
  })

  // Setup Workflows
  var workflowsSelectAll = deliveriesSelectAllG
    .selectAll('.workflow')
    .data(function(d) {
      return d.values
    })
  ...
  workflowsSelectAllG.each(function(d, i) {
  ...
  vehicleIconsG
    ...
    .attr('transform', function(d) {
      return 'translate(' + xScale(_now) + ',' + 0 + ')'
    })
    .on('click', function(delivery) {
      displayDetail(delivery)
    })

  var communicationSelectAll = deliveriesSelectAllG
...
function appendWorkflow(workflow, d) {
  ...
  if (utils.inSubstepLocation(d)) {
    ...
    if (startedAt === null && endedAt === null) {
      // workflow hasnt started yet
      workflow
        .append('line') // nonsearch notreached
        .attr('x1', function(d, i) {
          return xScale(
        ...
        .attr('x2', function(d, i) {
          return xScale(
            d.eta.getTime() +
              ...
              searchEPT * oneMinute -
              60000 // + _currentDeliveryDelayById[d.deliveryId] * 60000
          )
        })
        .attr('y2', function(d, i) {
          return 0
      ...
      workflow
        .append('line') // release notreached
        .attr('x1', function(d, i) {
          return xScale(
      ...
      workflow
        .append('line') // substep 2
        .attr('x1', function(d, i) {
          return xScale(nonsearchEnd)
        })
        .attr('y1', function(d, i) {
          return 0
        })
        .attr('x2', function(d, i) {
          return xScale(searchEnd - 60000)
        })
        .attr('y2', function(d, i) {
          return 0
        })
      ...
      workflow
        .append('line')
        .attr('x1', function(d, i) {
          return xScale(searchEnd)
        })
        .attr('y1', function(d, i) {
          return 0
        })
        .attr('x2', function(d, i) {
          return xScale(endedAt)
        })
        .attr('y2', function(d, i) {
          return 0
        })
    ...
    if (startedAt !== null && endedAt !== null) {
      ...
      workflow
        ...
        .attr('x2', function(d, i) {
          return xScale(_now.getTime() + EPT * oneMinute - 60000)
        })
        .attr('y2', function(d, i) {
          return 0.001
        }) // IMPORTANT  if its flat its not displayed
...
function appendCurrentWorkflowWithSubsteps(currentWorkflow, d) {
  ...
  if (currentSubStep === 1) {
    currentWorkflow
      ...
      .attr('class', function(d) {
        var state = utils.calculateDelayState(
          d['started-at'],
          _now,
          d.nonSearchEPT
        )
    ...
    currentWorkflow
      ...
      .attr('x2', function(d, i) {
        return xScale(_now.getTime() + nonsearchEPT * oneMinute - 60000)
      })
      .attr('y2', function(d, i) {
        return 0.001
      }) // IMPORTANT  if its flat its not displayed
    ...
    currentWorkflow
      .append('line') // search notreached
      .attr('x1', function(d, i) {
        return xScale(_now.getTime() + nonsearchEPT * oneMinute)
      })
      .attr('y1', 0)
      .attr('x2', function(d, i) {
        return xScale(
      ...
      .attr('class', function(d) {
        var estimated =
          d['started-at'].getTime() + (nonsearchEPT + searchEPT) * 60000
        if (estimated < _now) {
    ...
    currentWorkflow
      ...
      .attr('y1', 0)
      .attr('x2', function(d, i) {
        return xScale(_now.getTime() + _.sum(epts) * oneMinute - 60000)
      })
      .attr('y2', 0.01)
      .style('stroke-dasharray', '2, 2')
      .style('stroke-width', 4)
      .attr('class', function(d) {
        var estimated =
          ...
          (nonsearchEPT + searchEPT + releaseEPT) * 60000
        if (estimated < _now) {
          return 'workflow lateGradient'
        } else {
          return 'workflow onTimeGradient'
    ...
    currentWorkflow
      .append('line') // substep 1 complted
      .attr('x1', function(d, i) {
        return xScale(startedAt)
      })
      .attr('y1', function(d, i) {
        return 0
      })
    ...
    currentWorkflow
      .append('line') // part of substep 2 (that was is being completed)
      .attr('x1', function(d, i) {
        return xScale(nonsearchEnd)
      })
      .attr('y1', function(d, i) {
    ...
    // on right side of now
    currentWorkflow
      .append('line') // future end of substep 2
      .attr('x1', function(d, i) {
        return xScale(_now)
      })
      .attr('y1', function(d, i) {
      ...
      .attr('x2', function(d, i) {
        return xScale(_now.getTime() + searchEPT * oneMinute - 60000)
      })
      .attr('y2', function(d, i) {
        return 0.001
      }) // IMPORTANT  if its flat its not displayed
    ...
    currentWorkflow
      .append('line') // release notreached
      .attr('x1', function(d, i) {
        return xScale(_now.getTime() + searchEPT * oneMinute)
      })
      .attr('x2', function(d, i) {
        return xScale(
    ...
    currentWorkflow
      .append('line') // part of step 3 done
      .attr('x1', function(d, i) {
        return xScale(searchEnd)
      })
      .attr('y1', function(d, i) {
  ...
  currentWorkflow
    .append('svg:path') // LEFT BOOKEND IS AT BOTTOM SO IT DISPLAYS ON TOP
    .attr('d', function(d) {
      return customShapes['lBook'](4)
    })
    .attr('class', function(d) {
